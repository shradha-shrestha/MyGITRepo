      ...
      if (filters.length) {
        ...
        for (i = 0; i < filters.length; i++) {
          ...
          if (filter in this.filterClasses) {
              ...
              if (filter.match("^__") && filter.match("__$")) {
                  // It's text filter
                  var matchNameArr = [];
                  filter = filter.replace('__', '').replace('__', '');
                  $.each(this.filterClasses, function (name, val) {
                      if (name.indexOf(filter) != -1) {
                          matchNameArr.push(name);
                      }
                  });

                  for (var j = 0; j < matchNameArr.length; j++) {
                      activeFilters.push(this.filterClasses[matchNameArr[j]]);
                  }
              }
          }
        }
        ...
        if (mode == 'or' || activeFiltersLength == 1) {
          ...
          for (i = 1; i < activeFiltersLength; i++) {
            ...
            // add checking in case of shortestFilter is undefine
          if (shortestFilter != undefined) {
